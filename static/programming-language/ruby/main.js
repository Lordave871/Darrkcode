/* Minimal runtime for ruby page:
   - Loads details.json
   - Renders topics grid + notes panel
   - Cheat & interview modals wiring
   - Progress + Back/Start + search (ruby-search)
*/
(async function(){
  'use strict';
  const DETAILS_URL = './details.json';
  const PROG_KEY = 'ruby_done_v1';
  const NOTES_KEY = 'ruby_notes_v1';
  let DETAILS = {};
  ...
  function renderTopics(filter=''){
    ...
    Object.keys(DETAILS).forEach(k=>{
      ...
      card.dataset.key = k;
      card.innerHTML = `
        <div class="card-icon" aria-hidden="true">${escapeHtml((d.title||'')[0]||'R')}</div>
  ...
  function renderSavedNotesList(highlightKey){
    ...
    if(highlightKey){ const a = container.querySelector(`[data-key="${String(highlightKey)}"]`); if(a){ a.scrollIntoView({behavior:'smooth', block:'start'}); a.style.boxShadow='0 12px 36px rgba(204,52,45,0.12)'; setTimeout(()=> a.style.boxShadow='',900); } }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back: single history.back()
    const backBtn = document.getElementById('ruby-back');
    if (backBtn) backBtn.addEventListener('click', ev => { ev.preventDefault(); try{ history.back(); }catch(e){ console.warn('back failed', e); } });
    // Start: scroll & open first
    const startBtn = document.getElementById('ruby-start');
    if (startBtn) startBtn.addEventListener('click', ev => { ev.preventDefault(); try{ const grid = document.getElementById('topics-grid'); if(grid){ grid.scrollIntoView({ behavior: 'smooth', block: 'start' }); setTimeout(()=> { const first = grid.querySelector('.option-card'); if(first){ try{ first.scrollIntoView({ behavior: 'smooth', block: 'center' }); }catch(_){ } try{ first.focus && first.focus(); first.click(); }catch(_){} } }, 350); } }catch(err){ console.warn('start error', err); } });
  });

  // Wire the Ruby page Back button to the programming-language overview page.
  document.addEventListener('DOMContentLoaded', () => {
    const backBtn = document.getElementById('ruby-back') || document.getElementById('banner-back');
    if (backBtn) {
      backBtn.addEventListener('click', ev => {
        ev.preventDefault();
        try {
          window.location.href = '../programmingl_language.html';
        } catch (e) {
          /* ignore */
        }
      });
