(function(){
  ...
  const PROG_KEY = 'swift_done_v1';
  const NOTES_KEY = 'swift_notes_v1';
  let DETAILS = {};
  ...
  function renderTopics(filter=''){
    ...
    Object.keys(DETAILS).forEach(k=>{
      ...
      card.dataset.key = k;
      card.innerHTML = `
        <div class="card-icon" aria-hidden="true">${escapeHtml(((d.title||'')[0]) || 'S')}</div>
  ...
  document.addEventListener('DOMContentLoaded', ()=> {
    ...
    // Back/start handlers similar to other pages
    document.getElementById('swift-banner-back')?.addEventListener('click', ev=>{ ev.preventDefault(); try{ if(window.history && window.history.length>1) window.history.back(); else window.location.href = '../programmingl_language.html'; }catch(e){ try{ window.location.href = '../programmingl_language.html'; }catch(_){} } });
    document.getElementById('swift-banner-start')?.addEventListener('click', ev=>{ ev.preventDefault(); try{ const grid = document.getElementById('topics-grid'); if(grid){ grid.scrollIntoView({ behavior: 'smooth', block: 'start' }); setTimeout(()=> { const first = grid.querySelector('.option-card'); if(first){ try{ first.scrollIntoView({ behavior: 'smooth', block: 'center' }); }catch(_){ } try{ first.focus && first.focus(); first.click(); }catch(_){} } }, 350); } }catch(err){ console.warn('start error', err); } });
  });
  ...
  window.SWIFT = { DETAILS, renderTopics, renderTopicList, refreshProgressUI };
})();
