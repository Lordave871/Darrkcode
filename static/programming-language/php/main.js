/* Minimal runtime for php.html:
   - Loads details.json
   - Renders topics grid + topic list
   - Topic modal, notes autosave per-topic
   - Progress ring + Cheat/Interview modals
*/
(function(){
  'use strict';
  const DETAILS_URL = './details.json';
  const PROG_KEY = 'php_done_v1';
  const NOTES_KEY = 'php_notes_v1';
  let DETAILS = {};
  ...
  function renderTopics(filter=''){
    ...
    Object.keys(DETAILS).forEach(k=>{
      ...
      card.dataset.key = k;
      card.innerHTML = `
        <div class="card-icon" aria-hidden="true">${escapeHtml(((d.title||'')[0]) || 'P')}</div>
  ...
  function renderSavedNotesList(highlightKey){
    ...
    if(highlightKey){ const a = container.querySelector(`[data-key="${String(highlightKey)}"]`); if(a){ a.scrollIntoView({behavior:'smooth', block:'start'}); a.style.boxShadow='0 12px 36px rgba(127,63,191,0.14)'; setTimeout(()=> a.style.boxShadow='',900); } }
  }
  ...
  document.addEventListener('DOMContentLoaded', ()=> {
    ...
    // Back: try history.back() otherwise programming-language overview
    const backBtn = document.getElementById('php-back') || document.getElementById('banner-back');
    if (backBtn) backBtn.addEventListener('click', ev => {
      ...
      try {
        ...
        try { window.location.href = '../programmingl_language.html'; } catch (_) {}
      }
    });

    // Start: open first topic
    const startBtn = document.getElementById('php-start');
