(function(){
  ...
  const PROG_KEY = 'kotlin_done_v1';
  const NOTES_KEY = 'kotlin_notes_v1';
  let DETAILS = {};
  ...
  function renderTopics(filter=''){
    ...
    Object.keys(DETAILS).forEach(k=>{
      ...
      card.dataset.key = k;
      card.innerHTML = `
        <div class="card-icon" aria-hidden="true">${((d.title||'')[0]||'K')}</div>
  ...
  // expose for debug
  window.KOTLIN = { DETAILS, renderTopics, renderTopicList, refreshProgressUI };
})();
