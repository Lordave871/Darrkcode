/* Minimal runtime for typescript page:
   - Loads details.json
   - Renders topics grid + notes panel
   - Modal plumbing for cheat & interview
   - Progress ring and autosave notes
*/
(async function(){
  'use strict';
  const DETAILS_URL = './details.json';
  const PROG_KEY = 'ts_done_v1';
  const NOTES_KEY = 'ts_notes_v1';
  let DETAILS = {};
  ...
  function renderTopics(filter=''){
    const grid = $('topics-grid'); if(!grid) return;
    grid.innerHTML = '';
    Object.keys(DETAILS).forEach(k=>{
      ...
      card.dataset.key = k;
      card.innerHTML = `
        <div class="card-icon" aria-hidden="true">${escapeHtml((d.title||'')[0]||'T')}</div>
  ...
  function renderTopicList(filter=''){
    ...
    const keys = Object.keys(DETAILS).filter(k=>k!=='interview_index' && (!filter || ((DETAILS[k] && ((DETAILS[k].title||'') + (DETAILS[k].summary||'')))||'').toLowerCase().includes(filter.toLowerCase())));
    ...
    keys.forEach(k=>{
      const d = DETAILS[k]||{};
      const row = document.createElement('div'); row.style.display='flex'; row.style.justifyContent='space-between'; row.style.alignItems='center'; row.style.padding='6px'; row.style.borderRadius='6px'; row.style.cursor='pointer'; row.style.color='var(--muted)';
      ...
      const learn = makeBtn('Learn', ()=> openTopicModal(k));
      const note = makeBtn('Notes', ()=> openNotesFor(k));
  ...
  function openNotesFor(key){
    ...
    $('notes-text').value = text; $('notes-text').focus(); renderSavedNotesList();
  }
  ...
  function renderSavedNotesList(highlightKey){
    ...
    if(highlightKey){ const a = container.querySelector(`[data-key="${String(highlightKey)}"]`); if(a){ a.scrollIntoView({behavior:'smooth', block:'start'}); a.style.boxShadow='0 12px 36px rgba(49,120,198,0.12)'; setTimeout(()=> a.style.boxShadow='',900); } }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cheat-btn')?.addEventListener('click', ()=>{ try{ renderCheatTopicList(); const first = Object.keys(DETAILS).find(k=>k && k!=='interview_index'); if(first) selectCheatTopic(first); }catch(e){}; const m=$('cheat-modal'); if(m){ m.style.display='flex'; m.setAttribute('aria-hidden','false'); } });
    document.getElementById('interview-btn')?.addEventListener('click', ()=>{ try{ renderInterviewTopicList(); const first = Array.isArray(DETAILS.interview_index) && DETAILS.interview_index.length ? DETAILS.interview_index[0].key : Object.keys(DETAILS).find(k=>k && k!=='interview_index'); if(first) selectInterviewTopic(first); }catch(e){}; const m=$('interview-modal'); if(m){ m.style.display='flex'; m.setAttribute('aria-hidden','false'); } });
    document.getElementById('cheat-close')?.addEventListener('click', ()=>{ const m=$('cheat-modal'); if(m){ m.style.display='none'; m.setAttribute('aria-hidden','true'); }});
    document.getElementById('interview-close')?.addEventListener('click', ()=>{ const m=$('interview-modal'); if(m){ m.style.display='none'; m.setAttribute('aria-hidden','true'); }});
    document.getElementById('cheat-copy')?.addEventListener('click', ()=>{ const pre=$('cheat-pre'); if(!pre) return; navigator.clipboard?.writeText(pre.textContent||'').then(()=>{ const b=$('cheat-copy'); const old=b.textContent; b.textContent='Copied'; setTimeout(()=> b.textContent=old,900); }).catch(()=>alert('Could not copy')); });


    // Back button: only perform a single history.back() (no fallbacks)
    const backBtn = document.getElementById('ts-back');
    if (backBtn) {
      backBtn.addEventListener('click', (ev) => {
        ev.preventDefault();
        // single back navigation only
        try {
          history.back();
        } catch (err) {
          // no fallback navigation here — keep behavior single and predictable
          console.warn('history.back() failed', err);
        }
      });
    }

    // Start now: scroll topics into view and try opening the first card
      ...
      startBtn.addEventListener('click', (ev) => {
        ...
        try {
          ...
          if (grid) {
            ...
            setTimeout(() => {
              const first = grid.querySelector('.option-card');
              if (first) {
                try { first.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (_) {}
                try { first.focus && first.focus(); first.click(); } catch (_) {}
              }
            }, 350);
          }
        } catch (err) {
          console.warn('ts-start handler error', err);
        }
      });
    }

    // ensure existing search wiring still works (rebind if needed)
    const search = document.getElementById('topic-search');
    if (search && !search._ts_search_wired) {
      search.addEventListener('input', (e) => {
        window.dispatchEvent(new CustomEvent('ts-search', { detail: e.target.value.trim().toLowerCase() }));
      });
      search._ts_search_wired = true;
    }
  });
